...
def decode_access_token(token: str) -> dict:
    """Decodifica e valida um JWT (Legado/Local)"""
    try:
...
def validate_supabase_token(token: str):
    """
    Valida o token JWT de forma totalmente OFFLINE e ultra-rápida.
    Elimina a lentidão (F5/Reload) que ocorria ao validar via requisição web
    'supabase.auth.get_user(token)' a cada rota da API.
    """
    try:
        # A chave 'JWT Secret' do Supabase fica em seu .env, se não houver, ele tentará usar uma padrão.
        # Em produção é essencial que a SUPABASE_JWT_SECRET no env seja a chave JWT correta de seu painel.
        SUPABASE_JWT_SECRET = os.getenv("SUPABASE_JWT_SECRET", SECRET_KEY)
        
        # Validamos o Auth Header com o algoritmo padrão e ignoramos a checagem 'aud' pois varia entre versões do banco superior
        payload = jwt.decode(
            token, 
            SUPABASE_JWT_SECRET, 
            algorithms=["HS256"], 
        ...
        # O Payload JWT do Supabase é rico e tem tudo que precisamos localmente
        return {
            "sub": payload.get("sub"), # O ID uuid do usuário
            "email": payload.get("email"),
            "user_metadata": payload.get("user_metadata", {}),
            "role": payload.get("role", "authenticated")
        }
    except JWTError as jwt_err:
        print(f"[Security] Token Supabase rejeitado offline: {str(jwt_err)}")
        return None
    except Exception as e:
        print(f"[Security] Erro inesperado ao validar token local: {str(e)}")
        return None
