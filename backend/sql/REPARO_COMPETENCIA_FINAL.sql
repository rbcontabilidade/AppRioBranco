-- =========================================================================
-- SCRIPT DE CONSOLIDAÇÃO: GERAÇÃO DE COMPETÊNCIA E RECORRÊNCIA
-- Execute este script no Editor SQL do Supabase.
-- =========================================================================

-- 1. Garante colunas de recorrência
...
-- 2. Remove versões anteriores para evitar conflito de assinatura
DROP FUNCTION IF EXISTS public.gerador_nova_competencia(INTEGER, INTEGER);

-- 3. Cria a função principal com SECURITY DEFINER (ignora restrições de permissão do usuário executor)
...
DECLARE
    ...
    v_check RECORD;
BEGIN
    -- A. Cria ou recupera a Competência
    INSERT INTO rh_competencias (mes, ano, status)
    VALUES (v_mes, v_ano, 'ABERTA')
    ON CONFLICT (mes, ano) DO UPDATE SET status = 'ABERTA'
...
-- TESTE RÁPIDO: Se der erro aqui, o script falhou.
-- SELECT public.gerador_nova_competencia(1, 2026);
