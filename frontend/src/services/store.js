...
window.Store = {
    ...
    async deleteCompetencia(compId) {
        ...
        try {
            // Estratégia: Utilizar o backend Python (que cuida das execuções vinculadas)
    ...
    async deleteCargo(id) {
        ...
        getAuthBySession(sessionId) {
            ...
            if (isGerente || isAdminNome) {
                ...
                const adminScreens = ['dashboard', 'operacional', 'clientes', 'equipe', 'rotinas', 'marketing', 'settings', 'competencias', 'meu-desempenho'];
            ...
            if (auth.telas_permitidas.length === 0) {
                auth.telas_permitidas = ['operacional', 'meu-desempenho'];
            }
